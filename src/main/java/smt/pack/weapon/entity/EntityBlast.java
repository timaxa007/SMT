...
public class EntityBlast extends Entity implements IProjectile {
	...
	private int ticksInAir, power;

	public EntityBlast(final World world) {
		super(world);
		this.thrower = null;
		this.bullet = null;
	}

	public EntityBlast(final World world, final EntityLivingBase entity, final Bullet bullet, int power) {
		super(world);
		this.thrower = entity;
		this.bullet = bullet;
		this.power = power;
	...
	public void onImpact(MovingObjectPosition mop) {
		System.out.println("EntityBlast onImpact");
		switch(mop.typeOfHit) {
		case ENTITY:
			if (mop.entityHit != null && bullet != null)
				mop.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, getThrower()), bullet.getDamage() * power);
			System.out.println("EntityBlast onImpact, power" + power);
			break;
		case BLOCK:
			if (!worldObj.isRemote) {
				...
				if (block == Blocks.glass) {
					...
					worldObj.setBlock(mop.blockX, mop.blockY, mop.blockZ, Blocks.glass);
				else if (block == Blocks.cobblestone)
					worldObj.setBlock(mop.blockX, mop.blockY, mop.blockZ, Blocks.stone);
				else if (power >= 80)
