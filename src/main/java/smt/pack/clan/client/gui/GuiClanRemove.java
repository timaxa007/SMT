...
import smt.pack.clan.packet.MessageClanRemove;

public class GuiClanRemove extends GuiScreen {
	...
	public void actionPerformed(GuiButton button) {
		if (button.enabled) {
			...
			case 0:
				...
				break;
			case 1:
				MessageClanRemove mess = new MessageClanRemove();
	...
	public void mouseClicked(int mouseX, int mouseY, int button) {
		...
		/*
		if (button == 0) {
			boolean isClickedClanName = mouseX >= clanName.xPosition && mouseX < clanName.xPosition + clanName.width && mouseY >= clanName.yPosition && mouseY < clanName.yPosition + clanName.height ;
			if (isClickedClanName) {
				clanName.setCanLoseFocus(false);
				clanName.setFocused(true);
			} else {
				clanName.setCanLoseFocus(true);
				clanName.setFocused(false);
			}
		}
		*/
	}
	...
	public void drawScreen(int mouseX, int mouseY, float parTick) {
		...
		drawCenteredString(fontRendererObj, (
				StatCollector.translateToLocal(SMTClan.MODID + ".clan.remove")
