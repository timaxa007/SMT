package mods.timaxa007.pack.stock.lib.list;

import mods.timaxa007.pack.stock.lib.FoodForBlock;
import mods.timaxa007.pack.stock.lib.FoodForItem;
import mods.timaxa007.pack.stock.lib.GerminationPlants;
import mods.timaxa007.tms.lib.FluidFake;

public class Rhubarb {

	public static GerminationPlants herbal_rhubarb = new GerminationPlants("herbal_rhubarb").setName("rhubarb").setType("Herbal").setTexture("rhubarb").setPlantStats(3, 2, 1).setTemperatures(30.0F, 0.0F, 60.0F).setHumidity(30.0F, 0.0F, 60.0F);

}
