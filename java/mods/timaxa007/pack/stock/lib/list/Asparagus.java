package mods.timaxa007.pack.stock.lib.list;

import mods.timaxa007.pack.stock.lib.FoodForBlock;
import mods.timaxa007.pack.stock.lib.FoodForItem;
import mods.timaxa007.pack.stock.lib.GerminationPlants;
import mods.timaxa007.tms.lib.FluidFake;

public class Asparagus {

	public static GerminationPlants crop_asparagus = new GerminationPlants("crop_asparagus").setName("asparagus").setType("Crop").setTexture("asparagus").setPlantStats(3, 2, 1).setTemperatures(30.0F, 0.0F, 60.0F).setHumidity(30.0F, 0.0F, 60.0F);

}
