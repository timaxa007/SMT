...
public class ItemBlockCnstorAngle extends ItemBlock {
	...
	public boolean onItemUse(ItemStack is, EntityPlayer player, World world, int x, int y, int z, int meta, float hitX, float hitY, float hitZ) {
		if (!player.canPlayerEdit(x, y, z, meta, is)) {return false;}
			...
			world.setBlock(x, y, z, PackFurniture.proxy.block.cnstor_angle, 0, 3);
			TileEntity te = world.getTileEntity(x, y, z);
			NBTTagCompound tag = is.getTagCompound();
			if (te != null && te instanceof TileEntityAngleMod) {

				int l = MathHelper.floor_double((double)(player.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
				((TileEntityAngleMod)te).setRotation(l);

				if ((hitY >= 0.5F) && (hitY<=0.9F)) {
					((TileEntityAngleMod)te).setUPing(true);
				}
				if ((hitY <= 0.4F) && (hitY>=0.1F)) {
					((TileEntityAngleMod)te).setUPing(false);
				}
				if (hitY == 0.0F) {
					((TileEntityAngleMod)te).setUPing(true);
				}
				...
				if (tag != null) {
					if (tag.hasKey("Type")) ((TileEntityAngleMod)te).setType(tag.getInteger("Type"));
					if (tag.hasKey("Size")) ((TileEntityAngleMod)te).setSize(tag.getInteger("Size"));
				}
			}
			return true;
		}
