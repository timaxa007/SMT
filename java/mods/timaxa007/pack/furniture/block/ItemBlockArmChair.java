...
public class ItemBlockArmChair extends ItemBlock {
	...
	public boolean onItemUse(ItemStack is, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		if (!player.canPlayerEdit(x, y, z, side, is)) {return false;}
			...
			if (te != null && te instanceof TileEntityArmChair) {
				...
				((TileEntityArmChair)te).setRotation(l);

				if (tag != null) {
					if (tag.hasKey("Type")) ((TileEntityArmChair)te).setType(tag.getString("Type"));
					if (tag.hasKey("Size")) ((TileEntityArmChair)te).setSize(tag.getInteger("Size"));
				}
			}
			return true;
		}
	...
	public void addInformation(ItemStack is, EntityPlayer player, List list, boolean flag) {
		...
		if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) || Keyboard.isKeyDown(Keyboard.KEY_RSHIFT)) {
			if (tag != null) {
				if (tag.hasKey("Type")) list.add("Type: " + tag.getString("Type") + ".");
