...
import mods.timaxa007.pack.furniture.common.tile.TETable;
	...
	public boolean onItemUse(ItemStack is, EntityPlayer player, World world, int x, int y, int z, int meta, float hitX, float hitY, float hitZ) {
		if (!player.canPlayerEdit(x, y, z, meta, is)) {return false;}
			...
			if (te != null && te instanceof TETable) {
				...
				if (tag != null) {
					if (tag.hasKey("Type")) ((TETable)te).setType(tag.getInteger("Type"));
					if (tag.hasKey("Size")) ((TETable)te).setSize(tag.getInteger("Size"));
				}
			}
			return true;
		}
