...
import mods.timaxa007.pack.furniture.common.tile.TEArmChair;
	...
	public boolean onItemUse(ItemStack is, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		if (!player.canPlayerEdit(x, y, z, side, is)) {return false;}
			...
			if (te != null && te instanceof TEArmChair) {
				...
				if (tag != null) {
					if (tag.hasKey("Type")) ((TEArmChair)te).setType(tag.getInteger("Type"));
					if (tag.hasKey("Size")) ((TEArmChair)te).setSize(tag.getInteger("Size"));
				}
			}
			return true;
		}
