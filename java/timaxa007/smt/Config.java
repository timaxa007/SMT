...
public class Config {
	...
	public static void core(FMLPreInitializationEvent event) {
		...
		if (Loader.isModLoaded("Forestry")) is_mod_Forestry = true;
		if (Loader.isModLoaded("ThermalExpansion")) is_mod_ThermalExpansion = true;
		if (Loader.isModLoaded("Thaumcraft")) is_mod_Thaumcraft = true;
	...
	public static void pack(FMLPreInitializationEvent event) {
		...
		disable_pack_conjoint = cfg_pack.get("disable_pack", "conjoint,", false).getBoolean();
		...
		if (isPackConjoint) conjoint.preInit(event);
	...
	public static void verificationPack() {
		...
		if (!disable_pack_conjoint)
			conjoint = checkPack(CoreSMT.PATH_PACK + ".conjoint.PackConjoint");
		isPackConjoint = conjoint != null;
