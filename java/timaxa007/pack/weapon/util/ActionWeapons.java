...
public class ActionWeapons {
	...
	public static void onReload(EntityPlayer player, boolean buttonstate) {
		...
		if (current != null) {
			if (current.getItem() instanceof ItemWeapons) {
				((ItemWeapons)current.getItem()).reload(current, player.worldObj, player, buttonstate);
			}
		}
	}
	...
	public static void onMode(EntityPlayer player, boolean buttonstate) {
		...
		if (current != null) {
			if (current.getItem() instanceof ItemWeapons) {
				((ItemWeapons)current.getItem()).mode(current, player.worldObj, player, buttonstate);
			}
		}
	}
