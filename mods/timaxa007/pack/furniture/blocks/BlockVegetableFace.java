...
@SideOnly(Side.CLIENT) private Icon[][] icon_face;
...
public BlockVegetableFace(int id) {
super(id, Material.pumpkin);
setCreativeTab(PackFurniture.proxy.tabFurniturePack);
setHardness(0.5F);
setResistance(2.5F);
setStepSound(soundWoodFootstep);
setTextureName("pumpkin_side");
setUnlocalizedName("block_wood_blocks");
}

@Override
public TileEntity createNewTileEntity(World world) {return new TEBlockVegetableFace();}
...
if (te != null && te instanceof TEBlockVegetableFace) {
return addTag(world.getBlockId(x, y, z), ((TEBlockVegetableFace)te).getSubID(), ((TEBlockVegetableFace)te).getFace(), ((TEBlockVegetableFace)te).getColorBlock());
} else {
return addTag(world.getBlockId(x, y, z), 0, 0, 0xFFFFFF);
}
}
...
dropBlockAsItem_do(world, x, y, z, addTag(world.getBlockId(x, y, z), ((TEBlockVegetableFace)te).getSubID(), ((TEBlockVegetableFace)te).getFace(), ((TEBlockVegetableFace)te).getColorBlock()));
...
icon_array = new Icon[type_block.length][type_side.length];
icon_face = new Icon[type_block.length][type_face.length];
