...
wrd.setBlock(x, y, z, PackFurniture.proxy.block_table.blockID, 0, 3);
TileEntity te=wrd.getTileEntity(x, y, z);
NBTTagCompound tag = is.getTagCompound();
if(te!=null && te instanceof TETable) {

int l=MathHelper.floor_double((double)(player.rotationYaw*4.0F/360.0F)+0.5D)&3;
((TETable)te).setRotation(l);

if(tag!=null) {
((TETable)te).setType(tag.getInteger("Type"));
((TETable)te).setSize(tag.getInteger("Size"));
}else{
((TETable)te).setType(0);
((TETable)te).setSize(0);
}
}
return true;
}
